...
def test_simple_example():
    ...
    sim_ppr_pack = sim.element_sim(c1, c2,  alpha = 0.9, r = 1., r2 = None, rescale_path_type = 'max', ppr_implementation='prpack')
    sim_ppr_power_iteration = sim.element_sim(c1, c2,  alpha = 0.9, r = 1., r2 = None, rescale_path_type = 'max', ppr_implementation='power_iteration')
    
    assert_approx_equal(sim_ppr_pack, sim_ppr_power_iteration, significant=3)


def test_real_example_on_overlapping_community():
    ground_truth_community = json.load(open('ground_truth_community_Philosophy.json', 'r'))
